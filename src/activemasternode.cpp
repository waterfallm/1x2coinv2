...
// Copyright (c) 2017-2018 The 1x2coin developers
...
//
// Bootup the Masternode, look for a 1X2_COIN collateral input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if (status != ACTIVE_MASTERNODE_STARTED) {
        ...
        //     if (service.GetPort() != 9214) {
        //         notCapableReason = strprintf("Invalid port: %u - only 9214 is supported on mainnet.", service.GetPort());
        //         LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
        //         return;
        //     }
        // } else if (service.GetPort() == 9214) {
        //     notCapableReason = strprintf("Invalid port: %u - 9214 is only supported on mainnet.", service.GetPort());
        //     LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
        //     return;
        // }
...
bool CActiveMasternode::Register(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& errorMessage)
{
    ...
    // if (Params().NetworkID() == CBaseChainParams::MAIN) {
    //     if (service.GetPort() != 9214) {
    //         errorMessage = strprintf("Invalid port %u for masternode %s - only 9214 is supported on mainnet.", service.GetPort(), strService);
    //         LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
    //         return false;
    //     }
    // } else if (service.GetPort() == 9214) {
    //     errorMessage = strprintf("Invalid port %u for masternode %s - 9214 is only supported on mainnet.", service.GetPort(), strService);
    //     LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
    //     return false;
    // }
...
vector<COutput> CActiveMasternode::SelectCoinsMasternode()
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
		if (out.tx->vout[out.i].nValue == MasternodeCollateralLimitBM(chainActive.Height()) * COIN) { //exactly
