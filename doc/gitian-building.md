...
*Setup instructions for a gitian build of 1x2coin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the 1x2coin
...
- [Building 1x2coin](#building-1x2coin)
...
Clone the git repositories for 1x2coin and gitian and then checkout the 1x2coin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the 1x2coin repository
...
To build 1x2coin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the 1x2coin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/1x2coin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/1x2coin/1x2coin
...
For example:
```bash
URL=https://github.com/konez2k/1x2coin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit 1x2coin=${COMMIT} --url 1x2coin=${URL} ../1x2coin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit 1x2coin=${COMMIT} --url 1x2coin=${URL} ../1x2coin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit 1x2coin=${COMMIT} --url 1x2coin=${URL} ../1x2coin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/1x2coin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/1x2coin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/1x2coin-build.assert
```
...
[1x2coin/gitian.sigs](https://github.com/1x2coin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
